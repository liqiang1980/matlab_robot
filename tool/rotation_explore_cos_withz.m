...
rotate_actx = r2t(rotx(sin(1/100*mod(ind,6*HALFPI))*scale));
rotate_acty = eye(4);
rotate_actz = eye(4);
end
% if((mod(ind,6*HALFPI)<2*HALFPI)&&(mod(ind,6*HALFPI)>=HALFPI))
% rotate_actx = r2t(rotx(sin(pi/2+1/100*(mod(ind,2*HALFPI)))*scale));
% rotate_acty = eye(4);
% rotate_actz = eye(4);
% rotate_actx
% ind
...
rotate_actx = eye(4);
rotate_acty = r2t(roty(sin(1/100*(mod(ind,6*HALFPI)-2*HALFPI))*scale));
rotate_actz = eye(4);
end
% if((mod(ind,6*HALFPI)<=4*HALFPI)&&(mod(ind,6*HALFPI)>=3*HALFPI))
% rotate_actx = eye(4);
% rotate_acty = r2t(roty(sin(pi/2+1/100*(mod(ind,2*HALFPI)-2*HALFPI))*scale));
% rotate_actz = eye(4);
% rotate_acty
% ind
% end
...
rotate_actx = eye(4);
rotate_acty = eye(4);
rotate_actz = r2t(rotz(sin(1/100*(mod(ind,6*HALFPI)-4*HALFPI))*scale));
end
% if((mod(ind,6*HALFPI)<=6*HALFPI)&&(mod(ind,6*HALFPI)>=5*HALFPI))
% rotate_actx = eye(4);
% rotate_acty = eye(4);
% rotate_actz = r2t(rotz(sin(pi/2+1/100*(mod(ind,6*HALFPI)-4*HALFPI))*scale));
% end
